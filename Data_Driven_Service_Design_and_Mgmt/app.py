...
def _compute_contribution_scores(group_data: dict) -> dict:
    ...
    Components
    ----------
    Completion   0–30  Did they submit, and was it before the internal deadline?
    Effort       0–30  Word count relative to the group median (not a fixed cutoff)
    Substance    0–20  Sentence depth + repetition penalty
    Synthesis    0–20  Did they contribute to the synthesis round?

    Returns {member_name: {total, completion, effort, substance, synthesis,
                           word_count, median_wc, on_time, label, colour}}
    """
    ...
    internal_dl   = group_data.get("internal_deadline")   # ISO datetime str or None

    # Group median word count (only submitted members)
    word_counts = [
    ...
    for member in members:
        ...
        if sub:
            if internal_dl:
                try:
                    ...
                    completion    = 30 if on_time else 15
                    on_time_label = "On time" if on_time else "Submitted late"
                except Exception:
                    completion, on_time, on_time_label = 25, None, "No deadline set"
            else:
                completion, on_time, on_time_label = 25, None, "No deadline set"
        else:
            completion, on_time, on_time_label = 0, False, "Not submitted"

        # ── 2. Effort relative to median (0–30) ──────────────────────────────
        wc = sub.get("word_count", 0) if sub else 0
        if median_wc > 0 and wc > 0:
            ratio = wc / median_wc
            if   ratio >= 1.0:  effort = 30; effort_label = f"{wc} words — at or above group median"
            elif ratio >= 0.75: effort = 22; effort_label = f"{wc} words — slightly below group median"
            elif ratio >= 0.5:  effort = 15; effort_label = f"{wc} words — well below group median"
            elif ratio >= 0.25: effort = 7;  effort_label = f"{wc} words — much shorter than peers"
            else:               effort = 0;  effort_label = f"{wc} words — minimal contribution"
        elif wc > 0:
            effort = 20; effort_label = f"{wc} words (only submitter, no median yet)"
        else:
        ...
        if sub:
            ...
            raw_sentences = [s.strip() for s in re.split(r'[.!?]', text) if len(s.strip()) > 8]
            unique = set(s.lower() for s in raw_sentences)
            n_sent = len(raw_sentences)
            # Sentence depth score (up to 15)
            depth = min(n_sent, 10) * 1.5
            # Repetition penalty (up to -5)
            rep_ratio = 1 - (len(unique) / max(n_sent, 1))
            rep_penalty = int(rep_ratio * 5)
            substance = max(0, min(20, round(depth - rep_penalty)))
            if n_sent >= 8:   substance_label = f"{n_sent} sentences — well-developed argument"
            elif n_sent >= 4: substance_label = f"{n_sent} sentences — adequate depth"
            else:             substance_label = f"{n_sent} sentences — argument needs more development"
        else:
            substance = 0; substance_label = "No submission"

        # ── 4. Synthesis participation (0–20) ─────────────────────────────────
        if synth:
            synth_wc = len(synth.get("text", "").split())
            synthesis = 20 if synth_wc >= 30 else 10
            synthesis_label = f"Contributed {synth_wc} words to synthesis"
        else:
            synthesis = 0; synthesis_label = "Did not contribute to synthesis"

        total = completion + effort + substance + synthesis

        # Colour + label for the score
    ...
    with st.sidebar:
        ...
        if int_dl or ext_dl:
            ...
            if int_dl:
                ...
                past = datetime.now() > d
                flag = " ⚠️" if past else ""
                st.markdown(f'<div style="font-size:0.82rem;color:{"#E74C3C" if past else "#2C3E50"}">📝 Submissions: <strong>{d.strftime("%d %b %Y")}</strong>{flag}</div>', unsafe_allow_html=True)
            if ext_dl:
                d = datetime.fromisoformat(ext_dl)
                past = datetime.now() > d
                flag = " ⚠️" if past else ""
                st.markdown(f'<div style="font-size:0.82rem;color:{"#E74C3C" if past else "#2C3E50"}">🏁 Synthesis: <strong>{d.strftime("%d %b %Y")}</strong>{flag}</div>', unsafe_allow_html=True)
...
def page_alignment():
    ...
    with col1:
        ...
        st.markdown(
            ...
            'Completion 30 · Effort 30 · Substance 20 · Synthesis 20'
            '</div>',
            unsafe_allow_html=True,
        )
        ...
        for m in members:
            ...
            comp_html = "".join([
                ...
                f'{k[0].upper()}: {v}</span>'
                for k, v in [
                    ("completion", cs.get("completion", 0)),
                    ("effort",     cs.get("effort", 0)),
                    ("substance",  cs.get("substance", 0)),
                    ("synthesis",  cs.get("synthesis", 0)),
                ]
            ])
...
def page_done():
    ...
    if my_score:
        ...
        st.markdown(
            ...
            f'<div style="font-size:3rem;font-weight:800;color:{colour};line-height:1">{tot}</div>'
            f'<div style="font-size:1rem;color:{colour};font-weight:600;margin-bottom:4px">/100 — {label}</div>'
            f'<div style="font-size:0.85rem;color:#888">Group average: {group_avg}/100</div>'
            f'<div style="background:#E9ECEF;border-radius:99px;height:10px;margin:12px auto;max-width:300px;overflow:hidden">'
        ...
        components = [
            {
                ...
                "explain": "Were you on time? Max 30 pts for submitting before the internal deadline, 15 for late, 0 for missing.",
            },
            {
                "name":    "Effort",
                "max":     30,
                ...
                "explain": "How does your word count compare to the group median? Rewards relative contribution, not a fixed target.",
            },
            {
                "name":    "Substance",
                "max":     20,
                ...
                "explain": "Depth of argument: number of distinct sentences. Heavily repetitive or very short submissions score lower.",
            },
            {
                "name":    "Synthesis",
                "max":     20,
                ...
                "explain": "Did you contribute to the group synthesis round? Full marks for a substantive synthesis submission.",
            },
        ]
        ...
        for i, comp in enumerate(components):
            ...
            with cols[i % 2]:
                st.markdown(
                    ...
                    f'<span style="font-size:1rem">{comp["icon"]} <strong>{comp["name"]}</strong></span>'
